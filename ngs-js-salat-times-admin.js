...
window.ngs_js_salat_time.autocomplete = function (inp, arr) {
  ...
  inp.addEventListener("input", function(e) {
      ...
      a.setAttribute("id", this.id + "ngsjsst-autocomplete-list");
      a.setAttribute("class", "ngsjsst-autocomplete-items");
  ...
  inp.addEventListener("keydown", function(e) {
      var x = document.getElementById(this.id + "ngsjsst-autocomplete-list");
  ...
  function addActive(x) {
    ...
    x[currentFocus].classList.add("ngsjsst-autocomplete-active");
  }
  function removeActive(x) {
    /*a function to remove the "active" class from all autocomplete items:*/
    for (var i = 0; i < x.length; i++) {
      x[i].classList.remove("ngsjsst-autocomplete-active");
    }
  }
  function closeAllLists(elmnt) {
    /*close all autocomplete lists in the document,
    except the one passed as an argument:*/
    var x = document.getElementsByClassName("ngsjsst-autocomplete-items");
    for (var i = 0; i < x.length; i++) {
